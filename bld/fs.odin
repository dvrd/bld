...
read_entire_file :: proc(
    ...
    result, err := os2.read_entire_file_from_path(path, allocator)
    if err != nil {
...
write_entire_file :: proc(path: string, data: []u8) -> bool {
    err := os2.write_entire_file(path, data)
    if err != nil {
...
write_entire_file_string :: proc(path: string, content: string) -> bool {
    return write_entire_file(path, transmute([]u8)content)
}
...
file_exists :: proc(path: string) -> bool {
    _, err := os2.stat(path, context.temp_allocator)
    return err == nil
}
...
read_entire_dir :: proc(
    ...
    infos, read_err := os2.read_all_directory(f, allocator)
    if read_err != nil {
    ...
    os2.file_info_slice_delete(infos, allocator)
    return result, true
}
