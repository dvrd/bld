...
main :: proc() {
    ...
    // Get Odin compiler version info.
    odin_version := fmt.tprintf("%s", ODIN_VERSION)
    ...
    fmt.sbprintf(&sb, "BLD_ODIN_COMPILER     :: \"%s\"\n\n", odin_version)
    ...
    for ps in proc_sigs {
        ...
        fmt.sbprintf(&sb, "    %s: proc(%s)", field_name, ps.params)
        if len(ps.returns) > 0 {
    ...
    strings.write_string(&sb, "        fmt.eprintfln(\"[ERROR] Could not load bld library at '%s'\", dylib_path)\n")
    strings.write_string(&sb, "        os.exit(1)\n")
    strings.write_string(&sb, "    }\n\n")

    // Load global variable pointers.
    for gv in global_vars {
        fmt.sbprintf(&sb, "    %s_ptr, %s_ok := dynlib.symbol_address(_api.__handle, \"%s\")\n",
            gv.name, gv.name, gv.link_name)
        fmt.sbprintf(&sb, "    if %s_ok {\n", gv.name)
        fmt.sbprintf(&sb, "        %s = cast(^%s)%s_ptr\n", gv.name, gv.type_name, gv.name)
        fmt.sbprintf(&sb, "    }\n\n")
    }

    // Version mismatch warning.
    strings.write_string(&sb, "    // Warn if Odin compiler version differs.\n")
    strings.write_string(&sb, "    if BLD_ODIN_COMPILER != ODIN_VERSION {\n")
    strings.write_string(&sb, "        fmt.eprintfln(\n")
    strings.write_string(&sb, "            \"[WARNING] bld was compiled with Odin %s, you are using %s. ABI mismatch may cause crashes.\",\n")
    strings.write_string(&sb, "            BLD_ODIN_COMPILER, ODIN_VERSION,\n")
    strings.write_string(&sb, "        )\n")
    strings.write_string(&sb, "    }\n")
    strings.write_string(&sb, "}\n\n")
